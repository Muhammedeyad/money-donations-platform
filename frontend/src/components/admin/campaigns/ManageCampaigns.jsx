    ...
    return (
        ...
        <th className='flex gap-3 items-center justify-center font-bold'>
          {campaign.status == "Pending" ?<><button className="bg-green-500 text-xm rounded-lg text-white  hover:bg-green-400 w-30 p-3  " onClick={()=> updateApprove(campaign._id)}>Approve</button>
            <button className="text-xm bg-red-500 rounded-lg  text-white  hover:bg-red-400 p-3 w-15" onClick={()=> updateReject(campaign._id)}>Reject</button></>:campaign.status == "Approved"?     <button className="text-xm bg-red-500 rounded-lg  text-white  hover:bg-red-400 p-3 w-24" onClick={()=> updateClose(campaign._id)}>Close</button>: <span className={`${campaign.status == "Approved" ? "text-green-500":"text-red-500"}   px-3 py-2 `}>--  --</span>  }
